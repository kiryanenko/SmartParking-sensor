...
Была разработана функциональная схема МКПМ, представленная на рисунке Е.5. Данная схема описывает функциональный блок «PPCM» (Рисунок 10),
который используется при построении функциональной схемы МКПРСПМ.

Рисунок - УГО функционального блока PPCM

Ниже на рисунке Рисунок 11, показана функциональная схема МКПМ.

Рисунок - Функциональная схема МКПМ
...
-   Расширитель портов PCF8574, осуществляющий связь сенсора с МК, а
    ...
    «1» которого означает, что данное парковочное место свободно, а «0»
    - занято. Выход P2 выводит сигнал «Booked», логическая «1» которого
...
-   модулей МКПМ (PPCM) в количестве до 15 штук, устанавливаемых на
    каждое парковочное место;
-   клавиатура;
-   расширитель портов PCF8574 для подключения клавиатуры к
    микроконтроллеру;
-   дисплей;
-   часы реального времени DS3231.
...
Таблица - Описание выводов МК ATMEGA328
...
| 8                     | GND                   | Земля                 |
+-----------------------+-----------------------+-----------------------+
| 17                    | MOSI                  | Вход ведомого. Служит |
|                       |                       | для передачи данных   |
|                       |                       | от ведущего           |
|                       |                       | устройства ведомому   |
+-----------------------+-----------------------+-----------------------+
| 18                    | MISO                  | Выход ведомого.       |
|                       |                       | Служит для передачи   |
|                       |                       | данных от ведомого    |
|                       |                       | устройства ведущему   |
+-----------------------+-----------------------+-----------------------+
| 19                    | SCK                   | Последовательный      |
|                       |                       | тактовый сигнал       |
+-----------------------+-----------------------+-----------------------+
| 3-6,                  | PD1-PD6,              | Цифровые входы/выходы |
|                       |                       |                       |
| 11-19                 | PB0-PB5               |                       |
+-----------------------+-----------------------+-----------------------+
| 23-28                 | PC0-PC5               | Аналоговые            |
|                       |                       | входы/выходы          |
+-----------------------+-----------------------+-----------------------+
| 1                     | PC6                   | Перезапуск МК         |
+-----------------------+-----------------------+-----------------------+
| 2                     | PD0                   | Чтение (разъем для    |
|                       |                       | программатора)        |
+-----------------------+-----------------------+-----------------------+
| 9                     | PB6                   | Подключение внешнего  |
|                       |                       | резонатора (вход)     |
+-----------------------+-----------------------+-----------------------+
| 10                    | PB7                   | Подключение внешнего  |
|                       |                       | резонатора (выход)    |
+-----------------------+-----------------------+-----------------------+
...
крайне низком энергопотреблении, а также высокой помехоустойчивости. УГО
RFM95W представлено на рисунке 13, а описание выводов - в таблице 3.

Рисунок 13 - УГО приёмопередатчика RFM95W

Таблица - Описание выводов RFM95W
...
HC-SR04 не влияют источники света или цвет препятствия. Внешний вид
данного датчика показан на рисунке 15. На рисунке 16 представлена УГО датчика расстояния HC-SR04.
...
вид модуля часов реального времени представлен на рисунке Рисунок 18.
...
На рисунке Рисунок 19 показано УГО для часов реального времени DS3231SN.
В таблице Таблица 7 приводится описание выводов часов.
...
Данные схемы соответственно описывают блоки `PPCM` и `PPCM_A`, УГО которых
изображены на рисунке Рисунок 20. Данные модули отличаются от друг драга
-- модификацией расширителя порта PCF8574, на которой они построены.
...
Рисунок - УГО модулей PPCM и PPCM_A

Таблица - Описание выводов модулей PPCM / PPCM_A
...
Таблица - Подключение модуля RFM95W к МК ATMEGA328P
...
| 4, 5, 6, 7 -          | -                     | Выводы для            |
|                       |                       | подключения строк     |
| P0, P1, P2, P3        |                       | клавиатуры            |
+-----------------------+-----------------------+-----------------------+
| 8 - Vss               | -                     | Земля                 |
+-----------------------+-----------------------+-----------------------+
| 9, 10, 11, 12 -       | -                     | Выводы для            |
|                       |                       | подключения столбцов  |
| P4, P5, P6, P7        |                       | клавиатуры            |
+-----------------------+-----------------------+-----------------------+
| 14 - SCL              | 28 - PC5              | Последовательная      |
|                       |                       | линия синхронизации   |
+-----------------------+-----------------------+-----------------------+
| 15 - SDA              | 27 - PC4              | Последовательная      |
|                       |                       | линия данных          |
+-----------------------+-----------------------+-----------------------+
| 7 - V~DD\ ~           | -                     | Питание +5В           |
+-----------------------+-----------------------+-----------------------+
...
Таблица 11 - Подключение дисплея WAVGAT\_128x64 к МК ATMEGA328P
  ...
  Контакт WAVGAT\_128x64   | Контакт МК  | Роль
  ------------------------ + ----------- + --------------------------------------
  1 - GND                  | -           | Земля
  2 - V~CC~                | -           | Питание +5В
...
Заводская прошивка ATMEGA16U2 использует стандартные драйвера USB-COM
--- установка внешних драйверов не требуется, что делает принцип работы
микросхемы удобным в эксплуатации. В таблице 12 приведено
описание подключения драйвера к МК.
